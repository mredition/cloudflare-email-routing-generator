...
A minimal, professional README showing exactly which commands to run to use `create_routes.sh` locally or on a VPS. Commands are grouped as copy/paste blocks. Execute each block line-by-line in order.
...
Run the commands in each block in order. There is one blank line between each command for readability.
